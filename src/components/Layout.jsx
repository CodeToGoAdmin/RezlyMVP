...
export default function Layout() {
	...
	const [attendanceActiveTab, setAttendanceActiveTab] =
		useState("CHECK_IN"); // CHECK_IN | CHECK_OUT
	...
	const fetchQRCodes = async () => {
		...
		try {
			...
			toast.error(
				"حدث خطأ أثناء تحميل أكواد الحضور، حاول مرة أخرى"
			);
		} finally {
			setQrLoading(false);
		}
	...
	useEffect(() => {
		...
		if (path.startsWith("/dashboard/clients")) {
			...
			if (subPath && mapping[subPath]) {
				...
				navigate("/dashboard/clients/bookings", {
					replace: true,
				});
			}
		}
		...
		//طاقم العمل
			...
			if (subPath && mapping[subPath]) {
				...
				navigate("/dashboard/employees/staff", {
					replace: true,
				});
			}
		}
	...
	useEffect(() => {
		async function loadUser() {
			...
			if (saved) {
				...
				console.log(
					"تم تحميل المستخدم من localStorage:",
					parsed
				);
				return;
			}
	...
	useEffect(() => {
		const fetchUserProfile = async () => {
			...
			try {
				...
				console.error(
					"خطأ أثناء جلب بروفايل المستخدم:",
					error
				);
			}
		};
		...
		<div className="w-full h-screen flex bg-[#F8F8F8] overflow-hidden">
			...
			{showQrForThisUser && (
				<>
					{isQrPanelOpen && (
						<div
							className="fixed bottom-24 left-18 z-99"
							dir="rtl"
						>
