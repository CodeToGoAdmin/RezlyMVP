...
export default function Home() {
	...
	useEffect(() => {
		...
		const baseStats = {
			...
			totalSubscriptions:
				memberStats?.totalSubscriptions || 0,
			todaySubscriptions:
				memberStats?.todaySubscriptions || 0,
	...
	useEffect(() => {
		...
		const loadMembersStats = async () => {
			try {
				...
				const todaySubscriptions = members.filter(
					(m) => {
						const createdKey = getDateKey(
							m.createdAt
						);
				...
				const subscriptionsEndingSoon = members.filter(
					(m) => {
				...
				console.error(
					"خطأ أثناء تحميل إحصائيات الأعضاء للداشبورد:",
					err
				);
			} finally {
				if (!cancelled) {
