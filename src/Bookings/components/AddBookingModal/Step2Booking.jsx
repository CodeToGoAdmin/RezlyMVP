...
export default function Step2Booking({
	...
	useEffect(() => {
		setFormData((prev) => {
			if (
				JSON.stringify(prev.daysSchedule) ===
				JSON.stringify(daysSchedule)
			) {
				return prev;
			}
	...
	useEffect(() => {
		const handleClickOutside = (e) => {
			...
			const isInsideDuration =
				e.target.closest(".duration-dropdown");
			const isInsideReminder =
				e.target.closest(".reminder-dropdown");
			const isInsideStep2 =
				e.target.closest(".dropdown-step2");
